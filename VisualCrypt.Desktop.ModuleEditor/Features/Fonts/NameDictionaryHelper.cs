...
namespace VisualCrypt.Desktop.ModuleEditor.Features.Fonts
{
	internal static class NameDictionaryHelper
	{
		public static string GetDisplayName(LanguageSpecificStringDictionary nameDictionary)
		{
			...
			XmlLanguage userLanguage = XmlLanguage.GetLanguage(CultureInfo.CurrentCulture.IetfLanguageTag);
		...
		public static string GetDisplayName(IDictionary<CultureInfo, string> nameDictionary)
		{
			// Look for an exact match.
			string name;
			if (nameDictionary.TryGetValue(CultureInfo.CurrentCulture, out name))
			{
				return name;
			}
